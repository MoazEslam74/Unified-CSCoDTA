...
def get_target_molecule_graph(proteins, dataset):
    # لم نعد بحاجة لمسارات msa_path و contac_path
    target_graph = OrderedDict()
    for t in proteins.keys():
        g = target_to_graph(t, proteins[t])
        target_graph[t] = g

    return target_graph

def target_to_graph(target_key, target_sequence):
    target_size = len(target_sequence)
    
    # 1. استخراج الخصائص مباشرة من تسلسل الـ FASTA
    target_feature = target_to_feature(target_sequence)

    # 2. بناء روابط الرسم البياني (ربط كل حمض أميني بالذي يليه لعمل سلسلة)
    target_edge_index = []
    for i in range(target_size):
        target_edge_index.append([i, i]) # رابط ذاتي (Self-loop)
        if i < target_size - 1:
            target_edge_index.append([i, i+1]) # رابط مع الحمض التالي
            target_edge_index.append([i+1, i]) # رابط مع الحمض السابق
            
    target_edge_index = np.array(target_edge_index)

    return target_size, target_feature, target_edge_index

def target_to_feature(pro_seq):
    # استخراج الخصائص الفيزيائية والكيميائية والـ One-Hot (33 ميزة)
    other_feature = seq_feature(pro_seq)
    
    # إنشاء مصفوفة أصفار (21 ميزة) كبديل لـ PSSM للحفاظ على أبعاد النموذج المطلوبة (54)
    pssm_placeholder = np.zeros((len(pro_seq), 21))
    
    # دمج المصفوفتين ليصبح الإجمالي 54 ميزة مطابقة تماماً لما يتوقعه النموذج
    return np.concatenate((pssm_placeholder, other_feature), axis=1)

# (تجاهل أو احذف دالة PSSM_calculation و target_feature القديمة، فلن يستدعيها الكود بعد الآن)
